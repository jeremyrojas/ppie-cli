...
export async function runCompanion({ origin, version, clientName = DEFAULT_CLIENT_DISPLAY_NAME }) {
  ...
  const server = createServer(async (req, res) => {
    const browserPath = req.url === '/v1/pair' || req.url === '/v1/browser/poll' || req.url === '/v1/browser/result' || req.url === '/v1/browser/disconnect';
    let requestId = null;
    try {
      if (browserPath) {
      ...
      if (req.url === '/v1/browser/disconnect') {
        const disconnectSession = authorizeDisconnect(req, session, revokedSession);
        const envelope = validateEnvelope(body, ['browser.disconnect']);
        const key = `${disconnectSession.id}:${envelope.idempotencyKey}`;
        const result = disconnectSession.revoked
          ? replayIdempotent(dedupe, key, envelope)
          : await idempotent(dedupe, key, envelope, async () => {
              validateDisconnectPayload(envelope.payload);
              revokedSession = {
                id: disconnectSession.id,
                tokenHash: hashToken(disconnectSession.token),
                expiresAt: Date.now() + DEDUPE_TTL_MS,
              };
              session = null;
              if (pollWaiter) {
                clearTimeout(pollWaiter.timer);
                pollWaiter.reject(protocolError('CLI_NOT_PAIRED', 'Prompt Pie browser disconnected. Run "ppie pair".', {}, 401));
                pollWaiter = null;
              }
              rejectPending(createError('CLI_NOT_PAIRED', 'Prompt Pie browser disconnected. Run "ppie pair".'));
              return responseEnvelope(envelope, { disconnected: true });
            });
        writeJson(res, 200, await result);
        return;
      }
  ...
  function nextOperation(waitMs) {
    ...
    return new Promise((resolve, reject) => {
      ...
      pollWaiter = { resolve, reject, timer };
    });
  }
  ...
  function cleanup() {
    ...
    if (revokedSession && revokedSession.expiresAt < now) revokedSession = null;
...
function validateDisconnectPayload(payload) {
...
function replayIdempotent(cache, key, envelope) {
  const fingerprint = envelopeFingerprint(envelope);
  const existing = cache.get(key);
  if (!existing) throw protocolError('CLI_NOT_PAIRED', 'Prompt Pie browser is disconnected. Run "ppie pair".', {}, 401);
  if (existing.fingerprint !== fingerprint) throw protocolError('CLI_IDEMPOTENCY_CONFLICT', 'Idempotency key was reused with different request content.', {}, 409);
  return existing.promise;
}
...
function authorizeDisconnect(req, session, revokedSession) {
  const header = req.headers.authorization;
  const token = typeof header === 'string' && header.startsWith('Bearer ') ? header.slice(7) : '';
  try { return { ...validateSession(session, token), revoked: false }; } catch {}
  if (revokedSession && revokedSession.expiresAt >= Date.now() && safeTokenHash(token, revokedSession.tokenHash)) {
    return { id: revokedSession.id, revoked: true };
  }
  throw protocolError('CLI_NOT_PAIRED', 'Prompt Pie browser is disconnected. Run "ppie pair".', {}, 401);
}
...
function safeTokenHash(token, expectedHash) {
