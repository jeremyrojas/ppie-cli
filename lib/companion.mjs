...
import {
  consumeNonce, createPairingState, createSession, isAllowedRequestOrigin, validateAllowedOrigin,
  validateSession,
} from './pairing.mjs';
...
export async function pairCompanion({ origin, executable, version }) {
  const allowedOrigin = validateAllowedOrigin(origin);
  const current = await readHealthyState();
  let challenge;
  if (current && current.origin === allowedOrigin && current.protocol === PROTOCOL) {
    challenge = await privateRequest(current, '/v1/cli/operations', requestEnvelope('cli.pair', {}));
  } else {
    if (current) await requestShutdown(current);
    challenge = await spawnCompanion({ allowedOrigin, executable, version });
  }
  return {
    ...
    reused: Boolean(current && current.origin === allowedOrigin && current.protocol === PROTOCOL),
  };
}
...
export async function submitCompanionOperation(operation, { timeoutMs = REQUEST_TIMEOUT_MS } = {}) {
  const state = await readHealthyState();
  if (!state) throw createError('CLI_NOT_PAIRED', 'Local companion is unavailable. Run "ppie pair".');
  const envelope = requestEnvelope('cli.operation', { operation, timeoutMs });
  const response = await privateRequest(state, '/v1/cli/operations', envelope, timeoutMs + 1_000);
  return response;
}

export async function runCompanion({ origin, version }) {
  const allowedOrigin = validateAllowedOrigin(origin);
  const internalToken = randomBytes(32).toString('base64url');
  let pairing = createPairingState();
  let session = null;
  let closing = false;
  const queue = [];
  ...
  const state = { protocol: PROTOCOL, version, origin: allowedOrigin, host: LOOPBACK, port, pid: process.pid, startedAt, internalToken };
...
async function spawnCompanion({ allowedOrigin, executable, version }) {
  rmSync(COMPANION_STARTUP_FILE, { force: true });
  const child = spawn(process.execPath, [executable, '__companion'], {
    detached: true,
    stdio: 'ignore',
    env: { ...process.env, PPIE_COMPANION_ORIGIN: allowedOrigin, PPIE_COMPANION_VERSION: version },
  });
  child.unref();
...
async function readHealthyState() {
  const state = readState();
  if (!state) return null;
  try { await privateGet(state, '/v1/cli/status'); return state; } catch { return null; }
}
...
async function requestShutdown(state) {
  try { await privateRequest(state, '/v1/cli/operations', requestEnvelope('cli.shutdown', {})); } catch {}
  await delay(80);
}
